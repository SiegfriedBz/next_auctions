...
import {
  ...
  CreateAuctionParamsSchema,
} from "@/core/domains/auction";
import { SelectFor } from "../../../../../_components/select-for";
import {
  CATEGORY_OPTIONS,
  EXCLUDED_STATUSES_ON_CREATE,
  STATUS_OPTIONS,
  TOMORROW,
} from "../../../../../constants";

const formSchema = CreateAuctionParamsSchema;
type FormSchema = z.infer<typeof formSchema>;

const DEFAULT_VALUES: CreateAuctionParams = {
  title: "",
  description: "",
  startingPrice: 100,
...
export const CreateAuctionForm: FC = () => {
  ...
  const statusOptions = useMemo(() => {
    return STATUS_OPTIONS.filter(
      (o) => !EXCLUDED_STATUSES_ON_CREATE.includes(o.value),
    );
  }, []);
  ...
  const onSubmit = useCallback(
    async (values: FormSchema) => {
      try {
        ...
        if (result.success) {
          toast.success(t`You successfully created your auction!`);
